...
window.electron.bits.gotProducts((products) => {
    ...
    products.forEach(product => {
        ...
        cellInput(row, 'sku', sku);

        let { amount, type } = cost;
        cellInput(row, 'amount', amount);

        cellInput(row, 'in_development', in_development);
        cellInput(row, 'display_name', display_name);
        cellInput(row, 'expiration', expiration);
        cellInput(row, 'is_broadcast', is_broadcast);
...
function cellInput(row, name, value) {
    let cell = row.insertCell();
    ...
    switch (name) {
        ...
        inp.setAttribute('id', `bits_product_${name}`);
        inp.value = value;
